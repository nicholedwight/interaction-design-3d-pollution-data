...
function loadFile()
{
	jQuery.getJSON('./data/201244_data.json')
			.done(function(data)
			{
				...
				for(var cellKey in data)
				{
					...
					if(data[cellKey][constants.KEY_KM] > model[constants.KEY_MAX_KM][constants.KEY_TOTAL])
						model[constants.KEY_MAX_KM][constants.KEY_TOTAL] = data[cellKey][constants.KEY_KM];
				...
				{
					//load postal codes and show gui
					jQuery.getJSON('data/postalcodes_locations_xy.json')
					.done(function(data)
					{
			...
			.fail(function()
			{
		    	console.log( "error loading data 201244");
  			})
}
...
function loadData()
{
	...
	var numberOfWeeks = constants.WEEK_IDS.length;
	$.each(constants.WEEK_IDS, function(i, weekId)
	{
		console.log(weekId);
		jQuery.getJSON('data/' + weekId + '_data.json')
			.done(function(data)
			{
				...
				var expenditureCity = 0,
					expenditureProvince = 0,
					expenditureOthers = 0;
